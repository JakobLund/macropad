...
from utils.constants import TIMER_UPDATE_HW_INFO, COLOR_RED
...
class AppPad:
    ...
    def listen_to_usb(self, app):
        self.hw_info_app = app
        self.hw_info()

    def hw_info(self):
        print("oh no")
        self.add_timer(
            TIMER_UPDATE_HW_INFO,
            1,
            self.hw_info
        )
        #self.display_group[13].text
        if self.hw_info_app.display_group[13].text == "Hardware Info":
            if self.serial.connected and self.serial.in_waiting > 0:
                i = True
                msg = self.serial.read()
                self.hw_info_app.update_stuff(msg)
