...
class ByteCode(Command):
    """Sends a bytecode to host"""

    def __init__(self, byte_code_sender: ByteCodeSender, *bytecodes: ByteCodesEnum):
        """Initialize the ByteCode command.

        Args:
            ByteCode (Tuple[int, ...]): A tuple of ByteCodes to send.
        """
        super().__init__()
        self.bytecodes = bytecodes
        self.byte_code_sender = byte_code_sender

    def execute(self, app: BaseApp):
        """Send a keyboard press of the given keycodes."""
        for bytecode in self.bytecodes:
            self.byte_code_sender.send_byte_code(bytecode)

    def undo(self, app: BaseApp):
        """Send a keyboard release of the given keycodes."""
        #for keycode in reversed(self.bytecodes):
        #    app.macropad.keyboard.release(keycode)

    def __str__(self):
        return "{0}({1})".format(
            self.__class__.__name__,
            ", ".join(map(str, self.bytecodes)),
        )
...
class HwInfoRefreshException(Exception):
    """Raise this exception to switch the currently running app."""
...
class UpdateHWInfoCommand(Command):
    """A command to HW Info."""
...
class UpdateHwInfo(Command):
    """A command to update the color of a key."""
