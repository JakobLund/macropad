...
"""
from apps.hw import HwInfoApp
from apps.scripts import ScriptsApp
...
class HomeApp(KeyApp):
    ...
    def initialize_settings_dependent_keys(
        ...
        #hw_app = HwInfoApp(app_pad, settings)
        #cls.key_5 = Key(
        #    text="Hw Info", color=COLOR_FUNC, command=SwitchAppCommand(hw_app)
        #)
        ...
        scripts_app = ScriptsApp(app_pad, settings)
        cls.key_6 = Key(
            text="Scripts", color=COLOR_APPS, command=SwitchAppCommand(scripts_app)
        )
